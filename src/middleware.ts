...
export default auth((req) => {
      ...
      if (nextUrl.pathname === "/api/webhooks/stripe") {
            // Allow the webhook route to bypass authentication
            return;
      }

      if (nextUrl.pathname === "/api/webhooks/pusher") {
            // Allow the webhook route to bypass authentication
            return;
      }
      ...
      if (!isLoggedIn && !isPublicRoute) {
            if (nextUrl.pathname.startsWith("/seus-pedidos/rastreio/")) {
                  return; // Permitir acesso público à rota de detalhes do produto
            }

            if (nextUrl.pathname.startsWith("/canecas/detalhes/")) {
                  return; // Permitir acesso público à rota de detalhes do produto
            }
